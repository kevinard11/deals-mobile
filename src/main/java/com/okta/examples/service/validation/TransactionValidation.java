...
public class TransactionValidation {
    ...
    public void payTopup(JSONObject data){
        ...
        if (data.get("virtualAccount") == null || data.get("amount") == null){
            throw new PayOrderException("Please fill in all the forms.", HttpStatus.BAD_REQUEST);
        }

        if (!Pattern.matches(regex_va, ""+data.get("virtualAccount"))){
            throw new PayOrderException("A", HttpStatus.BAD_REQUEST);
        }
